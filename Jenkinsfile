        ...
        parallel(
          ...
          "WWW": {
            node(label: 'docker-1.13') {
              script {
                try {
                  sh '''docker run -i --net=host --name="$BUILD_TAG-www" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" ADDONS="${GIT_NAME}[test]" -e eeacms/www-devel /debug.sh bin/test -v -vv -s $GIT_NAME'''
                } finally {
          ...
          "KGS": {
            node(label: 'docker-1.13') {
              script {
                try {
                  sh '''docker run -i --net=host --name="$BUILD_TAG-kgs" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" ADDONS="${GIT_NAME}[test]" -e eeacms/kgs-devel /debug.sh bin/test --test-path /plone/instance/src/$GIT_NAME -v -vv -s $GIT_NAME'''
                } finally {
